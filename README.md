...
- 每次选择均可撤回，包含最终决选。
- 最终页直接展示完整晋级轨迹，并根据整场选择生成 3 首“可能错过的遗珠”；推荐支持网易云／QQ 音乐搜索、喜欢标记和“听过了，换一首”。
- 可生成 900 × 2320 PNG 结果图；图片包含 Top 1、逐轮等高列式晋级表、三首遗珠、曲库来源与本页二维码。
...
当前内地主站为 `https://dan-island-d8gwz7m0v7cc4c765-1422249946.tcloudbaseapp.com/`，排行榜 API 为 `https://dan-island-d8gwz7m0v7cc4c765.service.tcloudbase.com/api`。GitHub Pages 与腾讯云静态站点都可独立打开，不再互相跳转；两边的公开页面统一读写腾讯云 API，因此排行榜数据仍只有一份。所有新生成的二维码固定指向腾讯云主站。
...
`recommendation.js` 会读取每次选择、复活、淘汰赛阶段和最终名次，为越靠后的选择赋予越高权重。候选优先来自未进入 Top 16、未被用户主动保留的曲目，并通过风格相似度和多样性重排选出 3 首。`seedScore` 仅作为站内曝光先验，因此页面明确说明遗珠标签不是平台实时收藏量。

仓库中的 `song-vectors.js` 默认为空，推荐器会使用曲风、来源、演唱形式、发行类型和歌词主题组成的本地向量。维护者拥有合法的本地音频与歌词素材时，可以复制 `scripts/song-analysis-input.example.json`，安装 `laion-clap`、`librosa`、`sentence-transformers` 和 `scikit-learn`，再运行：

```powershell
python scripts/analyze_song_vectors.py scripts/song-analysis-input.json --clap-checkpoint D:\Models\music_audioset_epoch_15_esc_90.14.pt
```

脚本使用 LAION-CLAP 生成音乐听感 embedding、BGE-M3 生成歌词 embedding，并用 librosa 提取节奏与频谱特征；之后压缩、归一化并只写入 `song-vectors.js`。源音频与完整歌词不会进入网站或输出文件。
