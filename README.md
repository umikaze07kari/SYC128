...
- 64 首及以上采用 40 强路线，较小曲库采用 20 强路线。首轮以四选一快速海选，同来源歌曲和高种子歌曲会尽量分散；选择越快，后续二选一同组内越靠上，但不影响配对。
- “都不熟悉”只在第一轮出现；被跳过的组会增加复活名额，不会随机替用户选择。
- 海选后进入潮汐复活，再进行双歌对决；阶段页以“对阵双方 → 晋级者”的二进一比赛卡展示本轮结果。
- 每次选择均可撤回，包含最终决选。
- 最终页可横向翻看每轮二选一对阵，并生成适配手机纵横比的 900 × 1950 PNG 结果图；图片采用逐轮等高列式晋级表，字号与格高从左向右递增，底部内含本页二维码。
