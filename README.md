...
- 曲库共 140 首，已合并《纯妹妹》不同版本，并移除《珠玉》《有趣》的 Live 版本；新增独唱、合唱及十档综艺舞台分类。
- 专辑曲会获得轻量种子加成；4 首种子直达岛心，其余 136 首分为 34 组进行四选一。
- 首轮采用“高种子分层＋来源互斥＋组间强度平衡”：同专辑重复同组受到最高惩罚，分组顺序也会打散，避免连续出现数个特别难选的专辑组。
- “都不熟悉”只在第一阶段出现；跳过整组后不会随机代选，潮汐复活名额会自动增加。
- 海选胜者与复活歌曲凑齐四十强，随后进行 40→20、20→10、10→5 的双歌对决。
...
“蛋岛”主题使用仓库内的矢量素材：苔绿岛屿、巨树、蛋形石、紫色潮汐和等高线，并加入像素网格与电子状态标签。PK 卡片取消描边，使用按来源区分的浅莫兰迪半透明磨砂层。复活池采用五列纯歌名矩阵，正常流程约 102 首候选可以在现代手机的一屏内完成浏览。所有界面与结果图均不依赖外部字体或图片服务。

当前 140 首歌中有 97 首成功匹配并下载了 Apple Music/iTunes 公开封面，存放在 `assets/covers/`；其余 43 首会自动显示统一的矢量封面 `assets/cover-fallback.svg`。浏览网页时不会再请求 Apple、QQ 音乐或网易云音乐。
...
- 来源类型：`album`、`single`、`ost`、`live`、`soloWork`、`collabWork`、`variety`、`other`
- 演唱形式：`solo` 或 `collab`
